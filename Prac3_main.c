  ...
  while (1)
  {
	...
	// ADC to LCD; TODO: Read POT1 value and write to LCD
	 // Read the ADC value
	      ...
	      char adc_string[16];
	      snprintf(adc_string, sizeof(adc_string), "ADC Value: %lu", adc_value);
...
void EXTI0_1_IRQHandler(void)
{
    ...
    if ((current_time - last_interrupt_time) >= 100) // Adjust the debounce delay
    {
        // Toggle the LED frequency
        if (delay_t == 500)
        {
            delay_t = 1000; // Change the delay to 1 Hz
        }
        else
        {
            delay_t = 500; // Change the delay to 2 Hz
        }
...
void writeLCD(char *char_in){
	...
	lcd_command(CLEAR);
	 // Print the ADC value on the LCD
	  lcd_putstring(char_in); //display the ADC value
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val
	 uint32_t ccr_val;
	    // Calculate the CCR value using the equation
	    ccr_val = (adc_val * (htim3.Init.Period + 1)) / 4096;
	    return ccr_val;
	}
