...
//store which pattern is currently displayed
...
//  // TODO: Start timer TIM16 in interrupt mode
...
for (uint16_t i=0;i<6;i++){
  ...
  /* USER CODE END 2 */
    //check if it is pressed variables
	uint16_t  is_half=0;
  /* Infinite loop */
...
void TIM16_IRQHandler(void)
{
	...
	// TODO: Change to next LED pattern; output 0x01 if the read SPI data is incorrect
		...
		//check if address returned matches that in array
		if (patterns[current]==read_from_address(current)){
			//Display pattern
			GPIOB->ODR = read_from_address(
					current);}
	else{
		//indicate SPI failure
